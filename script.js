document.addEventListener("DOMContentLoaded", () => {
  ...
  if (sharePopup && shareBtn && shareBtnText) {
    ...
    document.addEventListener("mouseup", (event) => {
      setTimeout(() => {
        ...
        if (selectedText.length > 10) {
          ...
          if (left < 5) left = 5;
          if (left + sharePopup.offsetWidth > window.innerWidth - 5) left = window.innerWidth - sharePopup.offsetWidth - 5;
    ...
    shareBtn.addEventListener("click", async () => {
      const shareData = { title: document.title, text: `"${currentSelectedText}"`, url: window.location.href };
      if (navigator.share) {
        try { await navigator.share(shareData); } catch (err) {}
      } else if (navigator.clipboard) {
        try {
          await navigator.clipboard.writeText(`"${currentSelectedText}" - From ${window.location.href}`);
          shareBtnText.textContent = "Copied!";
          setTimeout(() => { shareBtnText.textContent = "Share"; }, 1500);
        } catch (err) { alert("Oops, couldn't copy the text."); }
      }
      sharePopup.classList.remove("visible");
    });
    document.addEventListener("mousedown", (event) => {
      if (!sharePopup.contains(event.target)) sharePopup.classList.remove("visible");
    });
  }

  if (navMenu && navToggle) {
    let touchStartX = 0, isSwiping = false;
    const swipeThreshold = 50, edgeZone = window.innerWidth * 0.2;
    const openMenu = () => navMenu.classList.add("show-menu");
    const closeMenu = () => navMenu.classList.remove("show-menu");
    document.addEventListener("touchstart", (e) => {
      const touch = e.touches[0];
      if (navMenu.classList.contains("show-menu") || (!navMenu.classList.contains("show-menu") && touch.clientX > window.innerWidth - edgeZone)) {
    ...
    document.addEventListener("touchend", (e) => {
      ...
      if (navMenu.classList.contains("show-menu") && swipeDistance > swipeThreshold) {
        closeMenu();
      } else if (!navMenu.classList.contains("show-menu") && touchStartX - touchEndX > swipeThreshold && touchStartX > window.innerWidth - edgeZone) {
        openMenu();
      }
      isSwiping = false;
    });
  ...
  function initializeChatBot() {
    ...
    const chatToggleButton = document.getElementById("chat-toggle-btn"), chatWindow = document.querySelector(".chat-window"), messagesContainer = document.querySelector(".chat-messages"), chatInput = document.getElementById("chat-input"), chatSendButton = document.getElementById("chat-send-btn");
    let initialMessageSent = false;
    
    chatToggleButton.addEventListener("click", () => {
        chatWindow.classList.toggle("open");
        if (chatWindow.classList.contains("open") && !initialMessageSent) {
            setTimeout(() => addMessage("Hello! I'm the Bay Breeze helper bot. How can I assist you? You can ask me about calculators, guides, or how to join.", "bot"), 300);
    ...
    const getBotResponse = (query) => {
        ...
        if (q.includes("hello") || q.includes("hi") || q.includes("hey")) return "Hello there! How can I help you navigate the Bay Breeze resources?";
        if (q.includes("calc") || q.includes("speedup") || q.includes("gear") || q.includes("shard") || q.includes("rune") || q.includes("skill book") || q.includes("pet") || q.includes("castle")) return `It sounds like you're looking for our calculators! We have a full suite of them on our <a href="/tools.html">Tools page</a>.`;
        if (q.includes("guide") || q.includes("strategy") || q.includes("f2p") || q.includes("resource") || q.includes("morale") || q.includes("relic") || q.includes("event")) return `Looking for strategies? Our <a href="/resources.html">Resources page</a> has in-depth guides on Hero Mastery, Kingdom Foundations, and more.`;
        if (q.includes("join") || q.includes("discord") || q.includes("guild")) return "We'd love to have you! You can <a href='https://discord.gg/pXaNHAbz2E' target='_blank'>Join Our Discord</a> to become part of the Bay Breeze community.";
        if (q.includes("leader") || q.includes("razor") || q.includes("officer") || q.includes("badsmoke")) return `Our Guild Master is Razor. You can find a full list of our R4 Officers on the <a href="/index.html#members">Home page</a>.`;
        if (q.includes("sky") || q.includes("first r4") || q.includes("first officer")) return "That's a fun bit of guild history! If you click on 'sky' on the homepage, you might find a special surprise. 😉";
        if (q.includes("bug") || q.includes("report") || q.includes("hr") || q.includes("suggestion") || q.includes("idea")) return `If you've found a bug or have a suggestion, please use the <a href="/hr-report.html">Report to HR form</a>. We appreciate your feedback!`;
