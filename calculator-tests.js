...
QUnit.test(
  "calculates points correctly and shows goal not reached message",
  function (assert) {
    ...
    assert.equal(
      message,
      "You need 1,390 more points to reach the goal.",
      'Correctly shows "goal not reached" message'
    );
  }
);
...
QUnit.test("shows congratulations message when goal is met", function (assert) {
  ...
  assert.equal(
    ...
    'Correctly shows "congratulations" message'
  );
});

QUnit.module("Castle Calculator", {
  before: async function () {
...
QUnit.test("calculates single level upgrade (15 to 16)", function (assert) {
  ...
  assert.ok(
    resultsHTML.includes("800,000"),
    "Correctly calculates wood/stone cost"
  );
});

QUnit.test(
  "calculates multi-level upgrade with mixed resources (40 to 42)",
  function (assert) {
    ...
    const resultsHTML = document.getElementById("results").innerHTML;
    assert.ok(
      resultsHTML.includes("199,400,000"),
      "Correctly calculates total wood"
    );
    assert.ok(
      resultsHTML.includes("34,200,000"),
      "Correctly calculates total rubies"
    );
    assert.ok(
      resultsHTML.includes("252"),
      "Correctly calculates total essence"
    );
  }
);
...
QUnit.test(
  "calculates resources for a single tier upgrade (0 to 1-0)",
  function (assert) {
...
QUnit.test(
  "calculates total resources from Tier 0 to Tier 4-0",
  function (assert) {
    document.getElementById("current-tier").value = "0";
    ...
    assert.ok(
      resultsHTML.includes(">9<"),
      "Correctly calculates total soulstones"
    );
  }
);
...
QUnit.test(
  "calculates total shards for Legendary hero from None to White ★★★★★",
  function (assert) {
    ...
    assert.ok(
      resultsHTML.includes("430"),
      "Correctly calculates total Legendary shards"
    );
  }
);

QUnit.test(
  "calculates total shards for Mythic hero from Gold ★★★★★ to White ★★★★★",
  function (assert) {
    ...
    const resultsHTML = calculator.querySelector(".results-section").innerHTML;
    assert.ok(
      resultsHTML.includes("720"),
      "Correctly calculates partial Mythic shards"
    );
  }
);
...
QUnit.test("calculates total food needed to level up a pet", function (assert) {
  ...
  assert.equal(
    ...
    "18,900",
    "Correctly calculates total food"
  );
});

QUnit.test(
  "calculates rarity promotion cost from Epic to Legendary",
  function (assert) {
    document.getElementById("rarity-promote-from").value = "epic";
    document.getElementById("calculate-rarity-promotion").click();
    const detailsHTML = document.getElementById(
      "rarity-promotion-details"
    ).innerHTML;
    assert.ok(
      detailsHTML.includes("5x copies of the same pet"),
      "Correctly shows cost for same pet"
    );
    assert.ok(
      detailsHTML.includes("15x random Rare pets"),
      "Correctly shows cost for random rare pets"
    );
  }
);

QUnit.test(
  "calculates star promotion cost for Legendary pet",
  function (assert) {
    ...
    const detailsHTML = document.getElementById(
      "promotion-cost-details"
    ).innerHTML;
    assert.ok(
      detailsHTML.includes("23x copies of the same pet"),
      "Correctly totals same pet copies"
    );
    assert.ok(
      detailsHTML.includes("65x random Rare pets"),
      "Correctly totals random rare pets"
    );
    assert.ok(
      detailsHTML.includes("13,000x essence"),
      "Correctly totals essence"
    );
  }
);
...
QUnit.test(
  "calculates the runes needed from level 1 to level 10",
  function (assert) {
    document.getElementById("current-level").value = "1";
    document.getElementById("target-level").value = "10";
    document.getElementById("calculate-button").click();
    assert.equal(
      document.getElementById("total-runes").textContent,
      "19,683",
      "Correctly calculates total runes for Lvl 10"
    );
  }
);
...
QUnit.test(
  "calculates total books needed from level 1 to 15",
  function (assert) {
    document.getElementById("current-level").value = "1";
    ...
    assert.equal(
      ...
      "25,000",
      "Correctly calculates total books"
    );
  }
);
...
QUnit.test(
  "calculates points and remaining time for Facilities tab",
  function (assert) {
    ...
    assert.ok(
      resultsSection.innerHTML.includes("10d 5h 30m"),
      "Correct Initial Time"
    );
    assert.ok(
      resultsSection.innerHTML.includes("8d 8h 0m"),
      "Correct Speedups Used Time"
    );
    assert.ok(
      resultsSection.innerHTML.includes("1d 21h 30m"),
      "Correct Remaining Time"
    );
    assert.ok(
      resultsSection.innerHTML.includes("360,000"),
      "Correct GAR/KVK Points"
    );
  }
);
...
QUnit.test(
  "calculates total shards needed from level 0 to 20",
  function (assert) {
    document.getElementById("current-level").value = "0";
    document.getElementById("target-level").value = "20";
    document.getElementById("calculate-button").click();
    assert.equal(
      document.getElementById("total-shards").textContent,
      "1,185",
      "Correctly calculates total shards"
    );
  }
);

QUnit.test(
  "calculates total shards for a partial upgrade (5 to 10)",
  function (assert) {
    ...
    assert.equal(
      document.getElementById("total-shards").textContent,
      "200",
      "Correctly calculates partial shard cost"
    );
  }
);
